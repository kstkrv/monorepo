...
import nanoglaw/types.{
  type LlmResponse, type Message, type NanoglawError, DecodeError, LlmResponse,
  MaxTokens, NetworkError, ProviderError, Stop, TextMessage, ToolCall,
  ToolResultMessage, ToolUse, ToolUseMessage,
}
...
pub fn parse_response(body: String) -> Result(LlmResponse, NanoglawError) {
  let choice_decoder = {
  ...
  case json_compat.parse(body, decoder) {
